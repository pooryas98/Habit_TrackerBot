...
async def select_habit_to_delete(update: Update, context: CallbackContext) -> int:
    ...
    if not update.message: return ConversationHandler.END # Should be triggered by message
...
async def ask_confirm_delete(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        reply_markup = keyboards.yes_no_keyboard(
            # Remove the extra underscore after the constant
...
async def perform_delete(update: Update, context: CallbackContext) -> int:
    ...
    job_queue = context.job_queue # Still need job_queue from context
    ...
    try:
        ...
        # 1. Try removing reminder from DB (uses global conn), get job_name
        job_name_to_remove = await db_manager.remove_reminder_by_habit_id(habit_id)

        # 2. If a job_name was associated, remove it from the queue
        job_removed = False
    ...
    if update.callback_query:
        ...
        try:
            await update.callback_query.edit_message_text(message)
             ...
             edit_failed = True
             if update.effective_message: # Try sending new if editing failed
...
def delete_habit_conv_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        ...
        states={
            ...
            CONFIRM_DELETE: [
                # Use startswith pattern for confirm delete callback
        ...
        fallbacks=[
            ...
            # Handle cancellation via button during selection phase too
            CallbackQueryHandler(cancel_delete, pattern=f"^{c.CALLBACK_CANCEL_DELETE}$")
            ],
         persistent=False,
